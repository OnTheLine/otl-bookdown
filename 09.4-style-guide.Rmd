...
This style guide was created primarily for the HTML web edition of *On The Line* at <http://OnTheLine.trincoll.edu>, and will require additional modifications prior to publishing the final PDF print edition. Also, to make the Web edition as accessible as feasible for visually-impaired readers, this book is continually tested and improved using the [Wave Accessibility Evaluation Tool](http://wave.webaim.org/). This publication has customized `style.css` to improve visual contrast. See [relevant code on GitHub](https://github.com/OnTheLine/otl-bookdown/blob/master/css/style.css#L7).
...
Using Bookdown version 0.16 as of January 2020, note numbering with customized code appears differently across editions, and needs to be addressed prior to final publication:
...
#### Tables {-}
Currently, this book uses Markdown table formatting, rather than a R code chunk solution, which means that tables are not auto-numbered, so numbers and cross-references must be inserted manually prior to final publication. Overall, Markdown seems to provide sufficient control for the types of tables in this publication, compared to R packages such as kable and huxtable.
...
**Table TODO #: Title Here (and insert blank line before table begins)**
