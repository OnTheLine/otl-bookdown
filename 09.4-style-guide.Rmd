...
Comment-out text
```
[//]: (TODO - this comment appears in the source, but not the book output)
```
...
- Insert the image using R code chunk <https://bookdown.org/yihui/bookdown/r-code.html>, and follow with an if-else statement from Michael Dorman, to display interactive version in HTML and static version in PDF.
- STILL TO DO:
...
```{r sample-static-image, echo=FALSE, fig.cap="(ref:sample-static-image)", fig_height=7}
 knitr::include_graphics("images/1937-HOLC-HartfordMapScan.jpg")
```
...
```{r vimeo-sample-vimeo, echo=FALSE, fig.cap="(ref:vimeo-sample-video)", auto.pdf=TRUE}
...
```{r youtube-sample-video, echo=FALSE, fig.cap="(ref:youtube-sample-video)", auto.pdf=TRUE}
...
```{r ctda-sample-video, echo=FALSE, fig.cap="(ref:ctda-sample-video)", auto.pdf=TRUE}
...
```{r kaltura-sample-video, echo=FALSE, fig.cap="(ref:kaltura-sample-video)", auto.pdf=TRUE}
...
```{r pdf-sample-local, echo=FALSE, fig.cap="(ref:pdf-sample-local)", auto.pdf=TRUE}
