...
For an em-dash, use three hyphens---like this---and compare with below.

For an en-dash, use only two hyphens--like this--and compare with above.
...
This book simplifies each chapter/section title references by adding a short ID name in curly brackets, like this: `# Defining City and Suburban Lines {#defining}`.
...
##### KNOWN ISSUES in this book re: footnotes/endnotes {-}
TODO: Using Bookdown version 0.16 as of January 2020, note numbering appears differently across editions, and needs to be addressed prior to final publication:
...
- But in the PDF edition, endnote renumbering begins only at each chapter, not subchapter, regardless of the custom CSS code.
...
4. Playable videos in Web edition, with static screenshots in PDF and MS Word editions
...
- To reduce the width (or height) of the output, add `out.width="50%"` to the code chunk, which works for Web edition and also PDF edition. If width is less than full, also insert `fig.align="center"`.
...
- In the body text, insert a dynamic reference to the figure in the same file, because PDF engine creates floating images that may be placed on the next page. Example of dynamic ref followed by footnote:  ...as shown in Figure \@ref(fig:sample-interactive).^[@clarkCharterOakCity1876]
...
- **TODO prior to final PDF edition**:
...
(ref:youtube-sample-video) Here's a sample YouTube clip caption, with option to add Markdown link (to the full video on CTDA). YouTube video formatting in HMTL seems to be cleaner than Vimeo formatting.
...
(ref:vimeo-sample-video) Here's a sample Vimeo clip caption, with option to add Markdown link (to the full video on CTDA).
...
Reminder: As of January 2020, CTDA and Kaltura video feeds both appear in the browser in autoplay mode, so do not embed these feeds directly in iframes in the book. Instead, upload a short relevant video clip to YouTube or Vimeo to display in the book, and insert a link "to the full video on CTDA or Kaltura" in the caption and footnote.
...
##### Sample table {-}
Sample body text immediately above the table, with optional links and footnote.^[@ilyankouMapFederalHOLC2017].
